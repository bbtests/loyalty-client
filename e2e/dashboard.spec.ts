...
test.describe("Loyalty Dashboard", () => {
  ...
  test("displays loyalty dashboard correctly", async ({ page }) => {
    ...
    await expect(
      page.getByText("Track your rewards and achievements"),
    ).toBeVisible();

    // Check action buttons
    await expect(
      page.getByRole("button", { name: "Make Purchase" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Simulate Achievement" }),
    ).toBeVisible();
  });
  ...
  test("simulates achievement correctly", async ({ page }) => {
    ...
    await expect(
      page.getByText("Spent over ₦50,000 in a single transaction"),
    ).toBeVisible();
